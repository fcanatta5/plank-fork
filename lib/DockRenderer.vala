...
namespace Plank {
  ...
  public class DockRenderer : Renderer {
    ...
    public override void draw (Cairo.Context cr, int64 frame_time) {
      ...
      draw_dock_background (main_cr, background_rect, x_offset, y_offset, frame_time);
#if BENCHMARK
    ...
    void draw_dock_background (Cairo.Context cr, Gdk.Rectangle background_rect, int x_offset, int y_offset, int64 frame_time) {
      unowned PositionManager position_manager = controller.position_manager;
      ...
      var draw_x = background_rect.x + x_offset;
      var draw_y = background_rect.y + y_offset;
      cr.set_source_surface (background_buffer.Internal, draw_x, draw_y);
      cr.paint ();

      if (theme.BackgroundPulseEnabled && theme.BackgroundPulseTime > 0 && theme.BackgroundPulseStrength > 0.0) {
        var pulse_duration = theme.BackgroundPulseTime * 1000;
        var pulse_phase = (frame_time % (int64) pulse_duration) / (double) pulse_duration;
        theme.draw_background_pulse (cr, draw_x, draw_y, background_rect.width, background_rect.height, pulse_phase);
      }
    }
    ...
    void animate_draw_value_for_item (DockItem item, DockItemDrawValue draw_value) {
      ...
      if (theme.HoverLiftEnabled && is_hovered && screen_is_composited && theme.HoverLiftAmount > 0.0 && !item.is_separator ()) {
        var lift_progress = 1.0;
        if (hover_time < max_hover_time)
          lift_progress = easing_for_mode (AnimationMode.EASE_OUT_CUBIC, hover_time, max_hover_time);
        y_offset += lift_progress * theme.HoverLiftAmount * icon_size / 10.0;
      }
    ...
    void draw_item (Cairo.Context cr, DockItem item, DockItemDrawValue draw_value, int64 frame_time) {
      ...
      // draw pulsing hover glow before the icon/active overlay
      unowned DockItem hovered_item = controller.window.HoveredItem;
      if (theme.HoverGlowEnabled && hovered_item == item && draw_value.opacity > 0.0) {
        var hover_color = theme.HoverGlowColor;
        if (hover_color.equal ({ 0.0, 0.0, 0.0, 0.0 }))
          hover_color = item.AverageIconColor;

        var pulse = 1.0;
        if (theme.HoverGlowPulseTime > 0)
          pulse = 0.65 + 0.35 * ((Math.sin (frame_time / (double) (theme.HoverGlowPulseTime * 1000) * 2.0 * Math.PI) + 1.0) / 2.0);

        theme.draw_hover_glow (item_buffer, draw_value.hover_region, hover_color,
                               theme.HoverGlowStrength * pulse * draw_value.opacity);
      }

      if (theme.HoverRingEnabled && hovered_item == item && draw_value.opacity > 0.0 && theme.HoverRingWidth > 0.0) {
        var ring_color = theme.HoverRingColor;
        if (ring_color.equal ({ 0.0, 0.0, 0.0, 0.0 }))
          ring_color = item.AverageIconColor;

        var ring_pulse = 1.0;
        if (theme.HoverRingPulseTime > 0)
          ring_pulse = 0.45 + 0.55 * ((Math.sin (frame_time / (double) (theme.HoverRingPulseTime * 1000) * 2.0 * Math.PI) + 1.0) / 2.0);

        theme.draw_hover_ring (item_buffer, draw_value.hover_region, ring_color,
                               ring_pulse * draw_value.opacity, theme.HoverRingWidth);
      }
      ...
      if (opacity > 0) {
        if (theme.ActivePulseEnabled && (item.State & ItemState.ACTIVE) != 0 && theme.ActivePulseTime > 0) {
          var pulse = (Math.sin (frame_time / (double) (theme.ActivePulseTime * 1000) * 2.0 * Math.PI) + 1.0) / 2.0;
          opacity = double.min (1.0, opacity + theme.ActivePulseStrength * pulse);
        }
        ...
        if (theme.ActiveHaloEnabled) {
          var halo_color = theme.ActiveHaloColor;
          if (halo_color.equal ({ 0.0, 0.0, 0.0, 0.0 }))
            halo_color = color;
          theme.draw_active_halo (item_buffer, draw_value.hover_region, halo_color, opacity * draw_value.opacity);
        }
      }
    ...
    protected override bool animation_needed (int64 frame_time) {
      ...
      if (theme.BackgroundPulseEnabled && theme.BackgroundPulseStrength > 0.0 && theme.BackgroundPulseTime > 0)
        return true;
    ...
    inline bool item_animation_needed (DockItem item, int64 render_time) {
      ...
      if (render_time - item.LastActive <= theme.ActiveTime * 1000)
        return true;
      if (theme.ActivePulseEnabled
          && (item.State & ItemState.ACTIVE) != 0
          && theme.ActivePulseStrength > 0.0)
        return true;
      if (theme.HoverGlowEnabled
          && controller.window.HoveredItem == item
          && theme.HoverGlowStrength > 0.0)
        return true;
      if (theme.HoverRingEnabled
          && controller.window.HoveredItem == item
          && theme.HoverRingWidth > 0.0)
        return true;
      if (show_notifications
