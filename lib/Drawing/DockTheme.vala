...
namespace Plank {
  ...
  public class DockTheme : Theme {
    ...
    [Description (nick = "hover-glow-enabled", blurb = "Whether hovered dock items draw a theme-controlled pulsing glow.")]
    public bool HoverGlowEnabled { get; set; }

    [Description (nick = "hover-glow-color", blurb = "The RGBA color of the hovered item glow. Transparent means use the item's average icon color.")]
    public Color HoverGlowColor { get; set; }

    [Description (nick = "hover-glow-size", blurb = "The hovered item glow radius multiplier relative to the hover region.")]
    public double HoverGlowSize { get; set; }

    [Description (nick = "hover-glow-strength", blurb = "The hovered item glow opacity multiplier, from 0 to 1.")]
    public double HoverGlowStrength { get; set; }

    [Description (nick = "hover-glow-pulse-time", blurb = "The hovered item glow pulse time in milliseconds.")]
    public int HoverGlowPulseTime { get; set; }

    [Description (nick = "active-pulse-enabled", blurb = "Whether active item backgrounds subtly pulse while active.")]
    public bool ActivePulseEnabled { get; set; }

    [Description (nick = "active-pulse-time", blurb = "The active item pulse time in milliseconds.")]
    public int ActivePulseTime { get; set; }

    [Description (nick = "active-pulse-strength", blurb = "The active item pulse opacity addition, from 0 to 1.")]
    public double ActivePulseStrength { get; set; }

    [Description (nick = "progress-color", blurb = "The color used for item progress bars. Transparent means use urgent/indicator color.")]
    public Color ProgressColor { get; set; }

    [Description (nick = "progress-background-color", blurb = "The color used for item progress bar backgrounds. Transparent means use default dark background.")]
    public Color ProgressBackgroundColor { get; set; }

    [Description (nick = "progress-glow-enabled", blurb = "Whether item progress bars draw a soft glow.")]
    public bool ProgressGlowEnabled { get; set; }

    [Description (nick = "progress-glow-strength", blurb = "The progress bar glow opacity multiplier, from 0 to 1.")]
    public double ProgressGlowStrength { get; set; }

    [Description (nick = "hover-ring-enabled", blurb = "Whether hovered dock items draw an animated neon ring.")]
    public bool HoverRingEnabled { get; set; }

    [Description (nick = "hover-ring-color", blurb = "The RGBA color of the hovered item ring. Transparent means use the item's average icon color.")]
    public Color HoverRingColor { get; set; }

    [Description (nick = "hover-ring-width", blurb = "The line width of the hovered item ring.")]
    public double HoverRingWidth { get; set; }

    [Description (nick = "hover-ring-pulse-time", blurb = "The hovered item ring pulse time in milliseconds.")]
    public int HoverRingPulseTime { get; set; }

    [Description (nick = "hover-lift-enabled", blurb = "Whether hovered dock items lift away from the screen edge.")]
    public bool HoverLiftEnabled { get; set; }

    [Description (nick = "hover-lift-amount", blurb = "Hovered item lift amount in tenths of a percent of IconSize.")]
    public double HoverLiftAmount { get; set; }

    [Description (nick = "active-halo-enabled", blurb = "Whether active dock items draw a crisp neon halo.")]
    public bool ActiveHaloEnabled { get; set; }

    [Description (nick = "active-halo-color", blurb = "The RGBA color of active item halos. Transparent means use active item color.")]
    public Color ActiveHaloColor { get; set; }

    [Description (nick = "active-halo-width", blurb = "The line width of active item halos.")]
    public double ActiveHaloWidth { get; set; }
    ...
    protected override void reset_properties () {
      ...
      HoverGlowColor = { 0.0, 0.0, 0.0, 0.0 };
      HoverGlowSize = 0.80;
      HoverGlowStrength = 0.0;
      HoverGlowPulseTime = 1400;
      ActivePulseEnabled = false;
      ActivePulseTime = 2400;
      ActivePulseStrength = 0.0;
      ProgressColor = { 0.0, 0.0, 0.0, 0.0 };
      ProgressBackgroundColor = { 0.0, 0.0, 0.0, 0.0 };
      ProgressGlowEnabled = false;
      ProgressGlowStrength = 0.0;
      HoverRingEnabled = false;
      HoverRingColor = { 0.0, 0.0, 0.0, 0.0 };
      HoverRingWidth = 1.0;
      HoverRingPulseTime = 1200;
      HoverLiftEnabled = false;
      HoverLiftAmount = 0.0;
      ActiveHaloEnabled = false;
      ActiveHaloColor = { 0.0, 0.0, 0.0, 0.0 };
      ActiveHaloWidth = 1.0;
    }
    ...
    public void draw_item_progress (Surface surface, int icon_size, Color color, double progress) {
      ...
      var bg_color = ProgressBackgroundColor;
      Cairo.Pattern bg_stroke;
      Cairo.Pattern bg_fill;

      if (!bg_color.equal ({ 0.0, 0.0, 0.0, 0.0 })) {
        bg_stroke = new Cairo.Pattern.rgba (bg_color.red, bg_color.green, bg_color.blue, double.min (1.0, bg_color.alpha + 0.18));
        bg_fill = new Cairo.Pattern.linear (0, base_y, 0, base_y + base_height);
        bg_fill.add_color_stop_rgba (0.0, bg_color.red, bg_color.green, bg_color.blue, bg_color.alpha * 0.70);
        bg_fill.add_color_stop_rgba (1.0, bg_color.red, bg_color.green, bg_color.blue, bg_color.alpha);
      } else {
        bg_stroke = new Cairo.Pattern.rgba (0.20, 0.20, 0.20, 0.9);
        bg_fill = new Cairo.Pattern.linear (0, base_y, 0, base_y + base_height);
      ...
      if (progress > 0) {
        ...
        var progress_color = ProgressColor;
        if (progress_color.equal ({ 0.0, 0.0, 0.0, 0.0 }))
          progress_color = color;

        if (ProgressGlowEnabled && ProgressGlowStrength > 0.0) {
          var glow = new Cairo.Pattern.rgba (progress_color.red, progress_color.green, progress_color.blue,
                                             progress_color.alpha * ProgressGlowStrength * 0.55);
          draw_rounded_line (cr, fill_x - line_width, fill_y - line_width,
                             fill_width + 2.0 * line_width, fill_height + 2.0 * line_width,
                             true, true, glow, glow);
        }
        ...
        var progress_stroke = new Cairo.Pattern.rgba (progress_color.red, progress_color.green, progress_color.blue, progress_color.alpha);
        var progress_fill = new Cairo.Pattern.linear (0, fill_y, 0, fill_y + fill_height);
        progress_fill.add_color_stop_rgba (0.0, progress_color.red, progress_color.green, progress_color.blue, progress_color.alpha * 0.78);
        progress_fill.add_color_stop_rgba (0.5, progress_color.red, progress_color.green, progress_color.blue, progress_color.alpha);
        progress_fill.add_color_stop_rgba (1.0, progress_color.red, progress_color.green, progress_color.blue, progress_color.alpha * 0.86);
    ...
    /**
     * Draws a pulsing hover halo for a dock item.
     *
     * @param surface the surface to draw onto
     * @param rect item hover region
     * @param color the halo color
     * @param opacity the final opacity multiplier
     */
    public void draw_hover_glow (Surface surface, Gdk.Rectangle rect, Color color, double opacity) {
      if (opacity <= 0.0 || rect.width <= 0 || rect.height <= 0)
        return;
      ...
      var center_x = rect.x + rect.width / 2.0;
      var center_y = rect.y + rect.height / 2.0;
      var radius = double.max (rect.width, rect.height) * HoverGlowSize;

      if (radius <= 0.0)
        return;

      var gradient = new Cairo.Pattern.radial (center_x, center_y, 0, center_x, center_y, radius);
      gradient.add_color_stop_rgba (0.00, color.red, color.green, color.blue, 0.45 * opacity);
      gradient.add_color_stop_rgba (0.35, color.red, color.green, color.blue, 0.22 * opacity);
      gradient.add_color_stop_rgba (1.00, color.red, color.green, color.blue, 0.0);

      cr.save ();
      cr.set_source (gradient);
      cr.arc (center_x, center_y, radius, 0, Math.PI * 2);
      cr.fill ();
      cr.restore ();
    }

    /**
     * Draws a crisp animated neon rectangle around a hovered item.
     */
    public void draw_hover_ring (Surface surface, Gdk.Rectangle rect, Color color, double opacity, double line_width) {
      if (opacity <= 0.0 || rect.width <= 0 || rect.height <= 0 || line_width <= 0.0)
        return;

      unowned Cairo.Context cr = surface.Context;
      var inset = line_width + 1.0;

      cr.save ();
      cr.set_line_width (line_width);
      cr.set_source_rgba (color.red, color.green, color.blue, color.alpha * opacity);
      draw_rounded_rect (cr, rect.x + inset, rect.y + inset,
                         double.max (1.0, rect.width - 2.0 * inset),
                         double.max (1.0, rect.height - 2.0 * inset),
                         TopRoundness, BottomRoundness, line_width);
      cr.stroke ();
      cr.restore ();
    }

    /**
     * Draws a neon halo around an active item.
     */
    public void draw_active_halo (Surface surface, Gdk.Rectangle rect, Color color, double opacity) {
      if (!ActiveHaloEnabled || opacity <= 0.0 || rect.width <= 0 || rect.height <= 0 || ActiveHaloWidth <= 0.0)
        return;

      unowned Cairo.Context cr = surface.Context;
      var inset = ActiveHaloWidth + 2.0;

      cr.save ();
      cr.set_line_width (ActiveHaloWidth);
      cr.set_source_rgba (color.red, color.green, color.blue, color.alpha * opacity);
      draw_rounded_rect (cr, rect.x + inset, rect.y + inset,
                         double.max (1.0, rect.width - 2.0 * inset),
                         double.max (1.0, rect.height - 2.0 * inset),
                         TopRoundness, BottomRoundness, ActiveHaloWidth);
      cr.stroke ();
      cr.restore ();
    }
    ...
    protected override void verify (string prop) {
      ...
      case "HoverGlowColor":
      case "ActivePulseEnabled":
      case "ProgressColor":
      case "ProgressBackgroundColor":
      case "ProgressGlowEnabled":
      case "HoverRingEnabled":
      case "HoverRingColor":
      case "HoverLiftEnabled":
      case "ActiveHaloEnabled":
      case "ActiveHaloColor":
        break;

      case "HoverGlowSize":
        if (HoverGlowSize < 0.0)
          HoverGlowSize = 0.0;
        break;

      case "HoverGlowStrength":
        if (HoverGlowStrength < 0.0)
          HoverGlowStrength = 0.0;
        else if (HoverGlowStrength > 1.0)
          HoverGlowStrength = 1.0;
        break;

      case "HoverGlowPulseTime":
        if (HoverGlowPulseTime < 0)
          HoverGlowPulseTime = 0;
        break;

      case "ActivePulseTime":
        if (ActivePulseTime < 0)
          ActivePulseTime = 0;
        break;

      case "ActivePulseStrength":
        if (ActivePulseStrength < 0.0)
          ActivePulseStrength = 0.0;
        else if (ActivePulseStrength > 1.0)
          ActivePulseStrength = 1.0;
        break;

      case "ProgressGlowStrength":
        if (ProgressGlowStrength < 0.0)
          ProgressGlowStrength = 0.0;
        else if (ProgressGlowStrength > 1.0)
          ProgressGlowStrength = 1.0;
        break;

      case "HoverRingWidth":
        if (HoverRingWidth < 0.0)
          HoverRingWidth = 0.0;
        break;

      case "HoverRingPulseTime":
        if (HoverRingPulseTime < 0)
          HoverRingPulseTime = 0;
        break;

      case "HoverLiftAmount":
        if (HoverLiftAmount < 0.0)
          HoverLiftAmount = 0.0;
        break;

      case "ActiveHaloWidth":
        if (ActiveHaloWidth < 0.0)
          ActiveHaloWidth = 0.0;
        break;
